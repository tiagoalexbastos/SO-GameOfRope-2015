...
void reviewNotes (unsigned int coachId)
{
  ...
  while (!chooseTeam[coachId]) {  
    pthread_cond_wait(&waitForNotice[coachId], &accessCR);
  }
...
void callContestants(unsigned int coachId)
{
  ...
  unsigned int selection[M];
  selectContestants(coachId,selection);
  unsigned int i;
  for(i = 0; i <  M; i++){
    joinTheTeam[coachId][selection[i]] = true;
    pthread_cond_signal(&waitForCommand[coachId][selection[i]]);
  }
